...
public class MasterSystemMk2 : IMasterSystemConsole
{
    ...
    public MasterSystemMk2(IZ80Cpu cpu, IMasterSystemIoManager io, IVdpPort vdp, IControllerPort controllers,
        ...
        ICpuLogger cpuLogger, IZ80InterruptManagement cpuInterruptManagement)
    {
        _cpu = cpu;
        _io = io;
        _vdp = vdp;
        ...
        _memory = memory;
        // TODO: Need a Main master system logger independent of cpu/io/vdp etc
    ...
    public void Run()
    {
        ...
        while (_running)
        {
            ...
            {
                // Leave this run.  Don't add a sleep here since the overheads in doing a sleep affect the timing
            }
    ...
    private void Reset()
    {
        ...
        _io.Reset();
        _vdp.Reset();
        _vdp.SetDisplayType(VdpDisplayType.Pal);
        _controllers.Reset();
        _debugConsole.Reset();
    }
}